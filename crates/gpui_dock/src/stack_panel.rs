...
use gpui_component::{ActiveTheme, AxisExt, Placement, h_flex};
...
impl StackPanel {
    ...
    /// Collect all TabPanels currently attached to this StackPanel (recursing into nested
    /// StackPanels).
    pub fn tab_panels(&self, cx: &App) -> Vec<Entity<TabPanel>> {
        let mut out = Vec::new();
        ...
        for panel in &self.panels {
            if let Ok(tab_panel) = panel.view().downcast::<TabPanel>() {
                ...
                out.extend(stack_panel.read(cx).tab_panels(cx));
            }
        }
...
impl Render for StackPanel {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        h_flex()
            ...
            .bg(cx.theme().tab_bar)
            .child(
                // Make the element id unique per StackPanel to avoid cross-panel state conflicts.
                ResizablePanelGroup::new(("stack-panel-group", view_entity_id))
