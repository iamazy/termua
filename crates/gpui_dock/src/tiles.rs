...
use gpui::{
    AnyElement, App, AppContext, Bounds, Context, CursorStyle, DismissEvent, Div, DragMoveEvent,
    Empty, Entity, EntityId, EventEmitter, FocusHandle, Focusable, InteractiveElement, IntoElement,
    MouseButton, MouseDownEvent, MouseUpEvent, ParentElement, Pixels, Point, Render, ScrollHandle,
...
// Namespace actions to avoid collisions with other tile implementations (e.g. gpui-component).
actions!(gpui_dock_tiles, [Undo, Redo]);
...
impl Tiles {
    ...
    /// Collect TabPanels currently attached to this Tiles container.
    pub fn tab_panels(&self, _cx: &App) -> Vec<Entity<TabPanel>> {
        self.panels
            .iter()
            .filter_map(|item| item.panel.view().downcast::<TabPanel>().ok())
            .collect()
    }
    ...
    fn render_drag_bar(
        ...
        h_flex()
            ...
            .map(|this| with_cursor(this, CursorStyle::ClosedHand))
            .hover(|s| with_cursor(s, CursorStyle::OpenHand))
            .active(|s| with_cursor(s, CursorStyle::ClosedHand))
            .on_mouse_down(
                MouseButton::Left,
...
fn with_cursor<T: Styled>(mut target: T, cursor: CursorStyle) -> T {
    target.style().mouse_cursor = Some(cursor);
    target
}
