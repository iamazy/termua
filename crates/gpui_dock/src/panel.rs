...
use gpui::{
    AnyElement, AnyView, App, AppContext, Context, Entity, EntityId, EventEmitter, FocusHandle,
...
/// The icon shown in a tab header.
///
/// - Use [`TabIcon::Monochrome`] for theme-tinted icons (e.g. `icons/*.svg`).
/// - Use [`TabIcon::ColoredSvg`] for multi-color SVGs that should preserve their original colors.
#[derive(Clone, Debug, PartialEq)]
pub enum TabIcon {
    /// Theme-tinted SVG icon (rendered as a monochrome alpha mask).
    Monochrome {
    ...
    ColoredSvg {
        /// Asset path, e.g. `icons/git-bash.svg`.
        path: SharedString,
    },
}
...
pub trait Panel: EventEmitter<PanelEvent> + Render + Focusable {
    ...
    /// The tooltip of the tab header, default is `None`.
    ///
    /// Used to provide additional context when hovering the tab header.
    fn tab_tooltip(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    /// The icon of the tab header, default is `None`.
    fn tab_icon(&self, cx: &App) -> Option<TabIcon> {
        None
    }
    ...
    /// When the user explicitly closes this Panel (e.g. via tab close button or a Close action),
    /// this will be called.
    ///
    /// Note: Panels can be temporarily removed/re-inserted during operations like tab drag/drop.
    /// Those flows should *not* call `on_close`.
    ...
    /// Context menu shown when right-clicking the tab header for this panel.
    ///
    /// Defaults to the same contents as `dropdown_menu` so existing panels automatically
    /// customize the tab header menu without changes.
    fn tab_context_menu(
        &mut self,
        menu: PopupMenu,
        window: &mut Window,
    ...
    /// The addition toolbar buttons of the panel used to show in the right of the title bar,
    /// default is `None`.
    fn toolbar_buttons(
        &mut self,
        window: &mut Window,
...
pub trait PanelView: 'static + Send + Sync {
    ...
    fn tab_context_menu(&self, menu: PopupMenu, window: &mut Window, cx: &mut App) -> PopupMenu;
...
impl<T: Panel> PanelView for Entity<T> {
    ...
    fn tab_tooltip(&self, window: &mut Window, cx: &mut App) -> Option<AnyElement> {
        self.update(cx, |this, cx| {
            this.tab_tooltip(window, cx).map(|el| el.into_any_element())
        })
    }
    ...
    fn tab_context_menu(&self, menu: PopupMenu, window: &mut Window, cx: &mut App) -> PopupMenu {
        self.update(cx, |this, cx| this.tab_context_menu(menu, window, cx))
    }
