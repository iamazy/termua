...
use gpui::{
    ...
    EventEmitter, InteractiveElement, IntoElement, ParentElement, Pixels, Render, SharedString,
    Styled, Subscription, WeakEntity, Window, actions, div, prelude::FluentBuilder,
};
...
// Namespace actions to avoid collisions with other dock implementations (e.g. gpui-component).
actions!(gpui_dock_dock, [ToggleZoom, ClosePanel]);
...
pub struct DockArea {
    id: SharedString,
    /// The version is used to special the default layout, this is like the `panel_version` in
    /// [`Panel`](Panel).
    ...
    /// Whether to show the toggle button per placement.
    toggle_button_visible: Edges<bool>,
...
impl DockItem {
        ...
        let tile_panel = cx.new(|cx| {
            ...
            for (ix, item) in items.clone().into_iter().enumerate() {
                match item {
                    ...
                    DockItem::Panel { view, .. } => {
                        let meta: TileMeta = metas[ix].into();
                        // Tiles expects each item to be a TabPanel (so it can manage tab
                        // UI/controls). Wrap a single PanelView into a
                        // TabPanel to satisfy this constraint.
                        let tab_panel: Arc<dyn PanelView> = Arc::new(cx.new(|cx| {
                            let mut tab_panel = TabPanel::new(None, dock_area.clone(), window, cx);
                            tab_panel.add_panel(view.clone(), window, cx);
                            tab_panel
                        }));
                        let tile_item = TileItem::new(tab_panel, meta.bounds).z_index(meta.z_index);
                        tiles.add_item(tile_item, dock_area, window, cx);
                    }
                    _ => {
    ...
    pub fn find_panel(&self, panel: Arc<dyn PanelView>, cx: &App) -> Option<Arc<dyn PanelView>> {
        match self {
            Self::Split { items, .. } => items
                .iter()
                .find_map(|item| item.find_panel(panel.clone(), cx)),
            Self::Tabs { view, .. } => view
                .read(cx)
                .panels
                .iter()
    ...
    pub fn add_panel(
        ...
        match self {
            ...
            Self::Split { view, items, .. } => {
                let stack_view = view.clone();
                // The DockItem tree can become stale after drag/drop re-layouts because the live
                // layout is view-driven (StackPanel/TabPanel entities mutate), while DockItem's
                // `items` is not automatically kept in sync. When adding a panel, always target
                // the live view tree; otherwise we can "resurrect" stale TabPanels by re-attaching
                // their views, resulting in duplicated tab groups that mirror the same panels.
                if let Some(tab_view) = stack_view.read(cx).left_top_tab_panel(false, cx) {
                    tab_view.update(cx, |tab_panel, cx| {
    ...
    /// Collect all TabPanels currently attached to this DockItem's live view tree.
    pub(crate) fn tab_panels(&self, cx: &App) -> Vec<Entity<TabPanel>> {
        match self {
            DockItem::Tabs { view, .. } => vec![view.clone()],
            DockItem::Split { view, .. } => view.read(cx).tab_panels(cx),
            DockItem::Tiles { view, .. } => view.read(cx).tab_panels(cx),
            DockItem::Panel { .. } => Vec::new(),
        }
    }
}
...
impl DockArea {
    ...
    /// Set locked state of the dock area, if locked, the dock area cannot be split or move, but
    /// allows to resize panels.
    ...
    pub fn set_toggle_button_visible(&mut self, visible: bool, _: &mut Context<Self>) {
        self.toggle_button_visible = Edges {
            top: visible,
            left: visible,
            bottom: visible,
            right: visible,
    ...
    /// Set the visibility of the toggle button for a specific placement.
    pub fn set_toggle_button_visible_for(
        &mut self,
        placement: DockPlacement,
        visible: bool,
        _: &mut Context<Self>,
        ...
        match placement {
            DockPlacement::Left => self.toggle_button_visible.left = visible,
            DockPlacement::Bottom => self.toggle_button_visible.bottom = visible,
            DockPlacement::Right => self.toggle_button_visible.right = visible,
            DockPlacement::Center => {}
        }
    }

    pub(crate) fn toggle_button_visible_for(&self, placement: DockPlacement) -> bool {
        match placement {
            DockPlacement::Left => self.toggle_button_visible.left,
            DockPlacement::Bottom => self.toggle_button_visible.bottom,
            DockPlacement::Right => self.toggle_button_visible.right,
            DockPlacement::Center => false,
        }
    }
    ...
    fn subscribe_item(&mut self, item: &DockItem, window: &mut Window, cx: &mut Context<Self>) {
        match item {
            ...
            DockItem::Tabs { view, .. } => {
                // TabPanel can be used directly (not only inside StackPanel/Tiles), e.g. in
                // examples. We must subscribe here so ZoomIn/ZoomOut works and
    ...
    pub fn visible_tab_panels(&self, cx: &App) -> Vec<Entity<TabPanel>> {
        // In zoom mode, only the zoomed view is visible. Avoid broadcasting to hidden panes.
        if let Some(zoom_view) = self.zoom_view.as_ref() {
            if let Ok(tab_panel) = zoom_view.clone().downcast::<TabPanel>() {
                return vec![tab_panel];
            }
            return Vec::new();
        }

        let mut out: Vec<Entity<TabPanel>> = Vec::new();

        out.extend(self.center.tab_panels(cx));
        ...
        out.into_iter()
            .filter(|tab_panel| !tab_panel.read(cx).is_collapsed())
            .collect()
    }
...
mod tests {
    ...
    #[test]
    fn translations_load_for_expected_keys() {
        // Default locale is "en".
        assert_eq!(t!("Dock.Close"), "Close");
        assert_eq!(t!("Dock.Close", locale = "zh-CN"), "关闭");
    }
    ...
    impl EventEmitter<PanelEvent> for FakePanel {}

    impl Focusable for FakePanel {
    ...
    fn dock_area_visible_tab_panels_excludes_collapsed_docks(cx: &mut gpui::TestAppContext) {
        ...
        cx.draw(
            ...
            |window, app| {
                ...
                // Center has two visible tab panes.
                let p1: Arc<dyn PanelView> = Arc::new(app.new(|cx| FakePanel::new("p1", cx)));
                let p2: Arc<dyn PanelView> = Arc::new(app.new(|cx| FakePanel::new("p2", cx)));
                let center = DockItem::h_split(
                    vec![
                        DockItem::tabs(vec![p1], &dock_weak, window, app),
                        DockItem::tabs(vec![p2], &dock_weak, window, app),
                    ],
                    &dock_weak,
                    window,
                    app,
                );
                dock_area.update(app, |dock, cx| dock.set_center(center, window, cx));

                // Closed bottom dock should not count as visible.
                let p3: Arc<dyn PanelView> = Arc::new(app.new(|cx| FakePanel::new("p3", cx)));
                let bottom_panel = DockItem::tabs(vec![p3], &dock_weak, window, app);
                dock_area.update(app, |dock, cx| {
                    dock.set_bottom_dock(bottom_panel, None, false, window, cx);
                });

                assert_eq!(dock_area.read(app).visible_tab_panels(app).len(), 2);

                // Open the dock; it becomes visible.
                dock_area.update(app, |dock, cx| {
                    let Some(bottom_dock) = dock.bottom_dock.as_ref() else {
                        panic!("bottom dock missing");
                    };
                    bottom_dock.update(cx, |bottom_dock, cx| {
                        bottom_dock.set_open(true, window, cx);
                    });
                });
                assert_eq!(dock_area.read(app).visible_tab_panels(app).len(), 3);

                div().size_full().child(dock_area)
            },
        );
    ...
    #[gpui::test]
    fn dock_toggle_buttons_render_with_debug_selectors(cx: &mut gpui::TestAppContext) {
        cx.update(|app| {
            gpui_component::init(app);
            crate::init(app);
        });
        ...
        window_cx.draw(
            ...
            |window, app| {
                ...
                let center_panel: Arc<dyn PanelView> =
                    Arc::new(app.new(|cx| FakePanel::new("center", cx)));
                let center = DockItem::tabs(vec![center_panel], &dock_weak, window, app);
                dock_area.update(app, |dock, cx| dock.set_center(center, window, cx));

                let left_panel: Arc<dyn PanelView> =
                    Arc::new(app.new(|cx| FakePanel::new("left", cx)));
                let left = DockItem::tabs(vec![left_panel], &dock_weak, window, app);
                dock_area.update(app, |dock, cx| {
                    dock.set_left_dock(left, None, true, window, cx)
                });

                let right_panel: Arc<dyn PanelView> =
                    Arc::new(app.new(|cx| FakePanel::new("right", cx)));
                let right = DockItem::tabs(vec![right_panel], &dock_weak, window, app);
                dock_area.update(app, |dock, cx| {
                    dock.set_right_dock(right, None, true, window, cx)
                });
        ...
        assert!(
            window_cx.debug_bounds("gpui-dock-toggle-left").is_some(),
            "expected left dock toggle button to be debuggable"
        );
        assert!(
            window_cx.debug_bounds("gpui-dock-toggle-right").is_some(),
            "expected right dock toggle button to be debuggable"
        );
    }
}
