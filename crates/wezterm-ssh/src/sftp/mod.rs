...
use crate::sftp::{
    ...
    types::{Metadata, OpenFileType, OpenOptions, RenameOptions, WriteMode},
};
...
impl SftpChannelError {
    pub fn is_not_found(&self) -> bool {
        match self {
            Self::Sftp(SftpError::NoSuchFile | SftpError::NoSuchPath) => true,
            #[cfg(feature = "libssh-rs")]
            Self::LibSsh(err) => libssh_sftp_error_code(err)
                .and_then(SftpError::from_error_code)
                .is_some_and(|err| matches!(err, SftpError::NoSuchFile | SftpError::NoSuchPath)),
            _ => false,
        }
    }
}
...
#[cfg(feature = "libssh-rs")]
fn libssh_sftp_error_code(err: &libssh_rs::Error) -> Option<i32> {
    let message = err.to_string();
    let marker = "Sftp error code ";
    let (_, code) = message.rsplit_once(marker)?;
    code.parse().ok()
}
