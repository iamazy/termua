...
type WriteFuture = Pin<Box<dyn Future<Output = io::Result<usize>> + Send + Sync + 'static>>;
type FlushFuture = Pin<Box<dyn Future<Output = io::Result<()>> + Send + Sync + 'static>>;
type CloseFuture = Pin<Box<dyn Future<Output = io::Result<()>> + Send + Sync + 'static>>;
...
#[derive(Default)]
struct FileState {
    f_read: Option<ReadFuture>,
    f_write: Option<WriteFuture>,
    f_flush: Option<FlushFuture>,
    f_close: Option<CloseFuture>,
}
