...
#[error(
    "host key mismatch for ssh server {remote_address}. Got fingerprint {key} instead of the \
     expected value from your known hosts file {file:?}."
)]
        ...
        match sess.is_known_server()? {
            ...
            libssh_rs::KnownHosts::Changed => {
                let file = self
                    ...
                    .and_then(|kh| kh.split_whitespace().next())
