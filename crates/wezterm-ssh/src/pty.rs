...
use crate::{
    session::{SessionRequest, SessionSender, SignalChannel},
    sessioninner::{ChannelId, ChannelInfo, DescriptorState},
                    ...
                    if !self.shown_accept_env_error {
                        log::warn!(
                            "ssh: setenv {}={} failed: {}. Check the AcceptEnv setting on the ssh \
                             server side. Additional errors with setting env vars in this session \
                             will be logged at debug log level.",
                            key,
                            val,
                            err
                        );
