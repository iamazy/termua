...
impl Config {
    ...
    pub fn for_host<H: AsRef<str>>(&self, host: H) -> ConfigMap {
        ...
        if needs_reparse {
            log::debug!(
                "ssh configuration uses options that require two-phase parsing, which isn't \
                 supported"
            );
        }
        ...
        for &t in tokens {
            if let Some(v) = token_map.get(t) {
                ...
                *value = value.replace(t, &self.resolve_local_host(true));
                ...
                // unset We don't directly support ProxyJump, and this %j token
                // referencing may technically put this into two-phase evaluation
                // territory which we don't support.
