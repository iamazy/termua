...
pub struct Storage<T> {
    ...
    /// Stable row ids, kept in lock-step with `inner`.
    stable_row_ids: Vec<i64>,
}
...
impl<T> Storage<T> {
        ...
        T: Default,
    {
        ...
        let mut storage = Storage {
            ...
            visible_lines,
            len: visible_lines,
            stable_row_ids: vec![0; visible_lines],
        };
        storage.reset_stable_row_ids();
        storage
    }

    fn reset_stable_row_ids(&mut self) {
        self.stable_row_ids.resize(self.inner.len(), 0);

        if self.len == 0 {
            return;
        }

        let history = self.len.saturating_sub(self.visible_lines) as i32;
        let top = Line(-history);
        let bottom = Line(self.visible_lines as i32 - 1);

        for (stable_id, line) in (top.0..=bottom.0).enumerate() {
            let line = Line(line);
            let idx = self.compute_index(line);
            self.stable_row_ids[idx] = stable_id as i64;
        }
    }
    ...
    pub fn truncate(&mut self) {
        ...
        self.stable_row_ids.truncate(self.len);
    }
        ...
        T: Default,
    {
        if self.len + additional_rows > self.inner.len() {
            ...
            self.stable_row_ids.resize(realloc_size, 0);
        }
    ...
    pub fn replace_inner(&mut self, vec: Vec<Row<T>>) {
        self.len = vec.len();
        self.inner = vec;
        self.stable_row_ids = vec![0; self.inner.len()];
        self.reset_stable_row_ids();
        self.zero = 0;
    }
    ...
    #[inline]
    pub(crate) fn stable_row_id_for_line(&self, line: Line) -> i64 {
        let idx = self.compute_index(line);
        self.stable_row_ids[idx]
    }

    #[inline]
    pub(crate) fn set_stable_row_id_for_line(&mut self, line: Line, id: i64) {
        let idx = self.compute_index(line);
        self.stable_row_ids[idx] = id;
    }
    ...
    fn rezero(&mut self) {
        ...
        self.stable_row_ids.rotate_left(self.zero);
        self.zero = 0;
    }
}
    ...
    T: Deserialize<'de>,
{
    ...
    {
        let owned = StorageSerdeOwned::<T>::deserialize(deserializer)?;
        let mut out = Storage {
            inner: owned.inner,
            zero: owned.zero,
            visible_lines: owned.visible_lines,
            len: owned.len,
            stable_row_ids: Vec::new(),
        };
        out.reset_stable_row_ids();
        Ok(out)
    }
}
...
mod tests {
    use crate::{
        grid::{
            GridCell,
            row::Row,
            storage::{MAX_CACHE_SIZE, Storage},
        },
        index::{Column, Line},
        term::cell::Flags,
    };
    ...
    fn grow_after_zero() {
        ...
        expected.stable_row_ids.resize(expected.inner.len(), 0);
    ...
    fn initialize() {
        ...
        let expected_storage = Storage {
            ...
            visible_lines: 0,
            len: 9,
            stable_row_ids: vec![0; 6 + expected_init_size],
        };
