...
#[test]
fn stable_row_ids_are_monotonic() {
    let mut grid = Grid::<usize>::new(5, 1, 3);

    let top = grid.topmost_line();
    let bottom = grid.bottommost_line();

    // Initial IDs should be strictly increasing top -> bottom.
    let mut prev = grid.stable_row_id_for_line(top);
    for line in (top.0 + 1)..=bottom.0 {
        let id = grid.stable_row_id_for_line(Line(line));
        assert_eq!(id, prev + 1);
        prev = id;
    }

    // After scrolling, IDs should still be strictly increasing and adjacent-diff == 1.
    ...
    // Reverse mapping should locate stable rows.
    let mid_line = Line(2);
    let mid_id = grid.stable_row_id_for_line(mid_line);
    assert_eq!(grid.line_for_stable_row_id(mid_id), Some(mid_line));
}
