...
pub struct Grid<T> {
    ...
    /// Stable row id of the topmost line in the buffer.
    #[cfg_attr(feature = "serde", serde(default))]
    stable_row_min: i64,
    /// Stable row id of the bottommost line in the buffer.
    #[cfg_attr(feature = "serde", serde(default))]
    stable_row_max: i64,
}
...
impl<T: GridCell + Default + PartialEq> Grid<T> {
    pub fn new(lines: usize, columns: usize, max_scroll_limit: usize) -> Grid<T> {
        Grid {
            ...
            columns,
            stable_row_min: 0,
            stable_row_max: lines.saturating_sub(1) as i64,
        }
    }
        ...
        D: PartialEq,
    {
        ...
        if self.max_scroll_limit == 0 {
            ...
            let assign_stable = region.start == 0 && region.end == Line(self.screen_lines() as i32);
            if assign_stable && positions != 0 {
                self.stable_row_min = self.stable_row_min.saturating_sub(positions as i64);
            }

            let mut stable = self.stable_row_min;
            for i in (0..positions).map(Line::from) {
                self.raw[i].reset(&self.cursor.template);
                if assign_stable {
                    self.raw.set_stable_row_id_for_line(i, stable);
                    stable += 1;
                }
            }
            ...
            for line in range.rev().map(Line::from) {
                ...
                if assign_stable {
                    self.raw.set_stable_row_id_for_line(
                        line,
                        self.stable_row_min + i64::from(line.0 - region.start.0),
                    );
                }
            }
        }
        ...
        D: PartialEq,
    {
        ...
        for i in (region.end.0 - positions as i32..region.end.0).map(Line::from) {
            self.raw[i].reset(&self.cursor.template);
            if assign_stable {
                self.stable_row_max = self.stable_row_max.saturating_add(1);
                self.raw.set_stable_row_id_for_line(i, self.stable_row_max);
            }
        }
        if assign_stable {
            self.recalc_stable_row_bounds();
        ...
        D: PartialEq,
    {
        ...
        // Re-initialize stable row ids for the visible viewport.
        self.stable_row_min = 0;
        self.stable_row_max = self.lines.saturating_sub(1) as i64;
        for i in 0..self.lines {
            self.raw
                .set_stable_row_id_for_line(Line(i as i32), i as i64);
        }
    }
}

impl<T> Grid<T> {
    fn recalc_stable_row_bounds(&mut self) {
        if self.total_lines() == 0 {
            self.stable_row_min = 0;
            self.stable_row_max = -1;
            return;
        }

        let top = self.topmost_line();
        let bottom = self.bottommost_line();
        self.stable_row_min = self.raw.stable_row_id_for_line(top);
        self.stable_row_max = self.raw.stable_row_id_for_line(bottom);
    }

    /// Returns a stable row id for the given line coordinate.
    ///
    /// This value is monotonic with vertical ordering: lines above have smaller ids.
    #[inline]
    pub fn stable_row_id_for_line(&self, line: Line) -> i64 {
        self.raw.stable_row_id_for_line(line)
    }

    /// Best-effort inverse of [`Grid::stable_row_id_for_line`].
    ///
    /// Since stable row ids are monotonic, this uses a binary search across the current buffer.
    pub fn line_for_stable_row_id(&self, stable_row_id: i64) -> Option<Line> {
        let total = self.total_lines();
        if total == 0 {
            return None;
        }

        // Fast path: stable id outside the current range.
        if stable_row_id < self.stable_row_min || stable_row_id > self.stable_row_max {
            return None;
        }

        let top = self.topmost_line().0;
        let mut lo: i32 = 0;
        let mut hi: i32 = total.saturating_sub(1) as i32;

        while lo <= hi {
            let mid = lo + (hi - lo) / 2;
            let line = Line(top + mid);
            let id = self.stable_row_id_for_line(line);
            match id.cmp(&stable_row_id) {
