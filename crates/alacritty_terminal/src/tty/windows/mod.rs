...
use crate::{
    ...
    tty::{
        ChildEvent, EventedPty, EventedReadWrite, Options, Shell, windows::child::ChildExitWatcher,
    },
};
...
type RawReadPipe = UnblockedReader<AnonRead>;
type RawWritePipe = UnblockedWriter<AnonWrite>;

pub type IoTap = Arc<dyn Fn(&[u8]) + Send + Sync + 'static>;

pub struct TappedReadPipe {
    inner: RawReadPipe,
    on_read: Option<IoTap>,
}

impl TappedReadPipe {
    fn new(inner: RawReadPipe, on_read: Option<IoTap>) -> Self {
        Self { inner, on_read }
    }

    #[inline]
    fn register(&mut self, poller: &Arc<Poller>, event: Event, mode: polling::PollMode) {
        self.inner.register(poller, event, mode);
    }
...
impl io::Read for TappedReadPipe {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        let n = self.inner.read(buf)?;
        if n != 0
            && let Some(tap) = self.on_read.as_ref()
        {
            tap(&buf[..n]);
        }
        Ok(n)
...
pub struct TappedWritePipe {
    inner: RawWritePipe,
    on_write: Option<IoTap>,
}

impl TappedWritePipe {
    fn new(inner: RawWritePipe, on_write: Option<IoTap>) -> Self {
        Self { inner, on_write }
    }

    #[inline]
    fn register(&self, poller: &Arc<Poller>, event: Event, mode: polling::PollMode) {
        self.inner.register(poller, event, mode);
    }
...
impl io::Write for TappedWritePipe {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        let n = self.inner.write(buf)?;
        if n != 0
            && let Some(tap) = self.on_write.as_ref()
        {
            tap(&buf[..n]);
        }
        Ok(n)
...
type ReadPipe = TappedReadPipe;
type WritePipe = TappedWritePipe;
...
pub fn new_with_taps(
    config: &Options,
    window_size: WindowSize,
    _window_id: u64,
    on_output: Option<IoTap>,
    on_input: Option<IoTap>,
) -> Result<Pty> {
    conpty::new_with_taps(config, window_size, on_output, on_input)
}

impl Pty {
    fn new(
        backend: impl Into<Backend>,
        conout: impl Into<RawReadPipe>,
        conin: impl Into<RawWritePipe>,
        child_watcher: ChildExitWatcher,
        on_output: Option<IoTap>,
        on_input: Option<IoTap>,
    ) -> Self {
        Self {
            ...
            conout: TappedReadPipe::new(conout.into(), on_output),
            conin: TappedWritePipe::new(conin.into(), on_input),
            child_watcher,
        }
    }
