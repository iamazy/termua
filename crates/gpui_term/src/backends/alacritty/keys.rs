...
pub fn to_esc_str(
    ...
    if let Some(esc_str) = manual_escape_str(keystroke.key.as_ref(), &modifiers, mode) {
...
fn manual_escape_str(
    key: &str,
    modifiers: &AlacModifiers,
    mode: &TermMode,
) -> Option<&'static str> {
    manual_basic_escape_str(key, modifiers)
        .or_else(|| manual_alt_screen_escape_str(key, modifiers, mode))
        .or_else(|| manual_app_cursor_escape_str(key, modifiers, mode))
        .or_else(|| manual_edit_escape_str(key, modifiers))
        .or_else(|| manual_function_escape_str(key, modifiers))
        .or_else(|| manual_caret_notation_escape_str(key, modifiers))
}

fn manual_basic_escape_str(key: &str, modifiers: &AlacModifiers) -> Option<&'static str> {
    match (key, modifiers) {
...
fn manual_alt_screen_escape_str(
    key: &str,
    modifiers: &AlacModifiers,
    mode: &TermMode,
) -> Option<&'static str> {
    ...
    match (key, modifiers) {
        ("home", AlacModifiers::Shift) => Some("\x1b[1;2H"),
        ("end", AlacModifiers::Shift) => Some("\x1b[1;2F"),
        ("pageup", AlacModifiers::Shift) => Some("\x1b[5;2~"),
        ("pagedown", AlacModifiers::Shift) => Some("\x1b[6;2~"),
        _ => None,
    }
}

fn manual_app_cursor_escape_str(
    key: &str,
    modifiers: &AlacModifiers,
    mode: &TermMode,
) -> Option<&'static str> {
    if modifiers != &AlacModifiers::None {
        return None;
    }
...
fn manual_edit_escape_str(key: &str, modifiers: &AlacModifiers) -> Option<&'static str> {
    match (key, modifiers) {
...
fn manual_function_escape_str(key: &str, modifiers: &AlacModifiers) -> Option<&'static str> {
    if modifiers != &AlacModifiers::None {
        return None;
    }
...
fn manual_caret_notation_escape_str(key: &str, modifiers: &AlacModifiers) -> Option<&'static str> {
    match (key, modifiers) {
