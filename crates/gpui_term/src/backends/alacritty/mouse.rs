...
pub fn scroll_report(
    ...
    if mode.intersects(TermMode::MOUSE_MODE) {
        ...
        .map(|report| repeat_n(report, max(scroll_lines, 1) as usize))
    } else {
        None
    }
}
...
pub fn grid_point_and_side(
    ...
    let last_col = GridCol(cur_size.num_columns().saturating_sub(1));
...
/// Generate the bytes to send to the terminal, from the cell location, a mouse event, and the
/// terminal mode
fn mouse_report(
    point: AlacPoint,
